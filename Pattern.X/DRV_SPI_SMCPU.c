	...
	switch (GLCD_Info.State)
	{
                case GLCD_SS_WAIT:
                        if(G_SS == 0)
                        {
                            ...
                            SPI1STATbits.SPIROV = 0;
                            Temp = SPI1BUF;
                            //write to SPI buffer for next set of 8 bytes
//                            while(!SPI1STATbits.SPITBF)
//                            {
//                                SPI1BUF = 0x11;					// write to buffer for TX
//                            }
...
//                if(checksum_G == ((UINT16)(GLCD_Info.Rx_Message_Buffer[MAX_G_PACKET_LEN-1]<<8)+(GLCD_Info.Rx_Message_Buffer[MAX_G_PACKET_LEN-2])))
                if(1)
                {
                    Process_Rx_Packet();
                    Update_GLCD = 1;
...
void Process_Rx_Packet(void)
{
    ...
    COM_DS_pkt_error_cnt.Byte.Lo = GLCD_Info.Rx_Message_Buffer[((CPU_PACKET_LEN*2) + SMCPU_PACKET_LEN)];
    COM_DS_pkt_error_cnt.Byte.Hi = GLCD_Info.Rx_Message_Buffer[((CPU_PACKET_LEN*2) + SMCPU_PACKET_LEN) + 1];
    COM_US_pkt_error_cnt.Byte.Lo = GLCD_Info.Rx_Message_Buffer[((CPU_PACKET_LEN*2) + SMCPU_PACKET_LEN) + 2];
    COM_US_pkt_error_cnt.Byte.Hi = GLCD_Info.Rx_Message_Buffer[((CPU_PACKET_LEN*2) + SMCPU_PACKET_LEN) + 3];
    ...
    SMCPU_sysinfo.SMCPU_Checksum.DWord.LoWord.Byte.Lo  =  GLCD_Info.SMCPU_Data[SMCPU_PK_OFFSET_S_CRC];
    SMCPU_sysinfo.SMCPU_Checksum.DWord.LoWord.Byte.Hi  =  GLCD_Info.SMCPU_Data[SMCPU_PK_OFFSET_S_CRC + 1];
    SMCPU_sysinfo.SMCPU_Checksum.DWord.HiWord.Byte.Lo  =  GLCD_Info.SMCPU_Data[SMCPU_PK_OFFSET_S_CRC + 2];
    SMCPU_sysinfo.SMCPU_Checksum.DWord.HiWord.Byte.Hi  =  GLCD_Info.SMCPU_Data[SMCPU_PK_OFFSET_S_CRC + 3];
    SMCPU_sysinfo.Event_Logger_ID = GLCD_Info.Rx_Message_Buffer[SMCPU_EVENT_ID];
    if(GLCD_Info.Rx_Message_Buffer[COMM_SRC_ID]==0x3c)
    {
        Packet_src = 1;
    }
    else{
